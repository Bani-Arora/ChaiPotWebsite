...
function DeleteAccount() {
  ...
  const onSubmit = async (data) => {
    ...
    await axios.post("//localhost:4001/user/deleteAccount",userInfo)
    .then((res)=>{
  ...
  return (
    ...
    <div className="card bg-neutral-200 shrink shadow-2xl rounded-md dark:bg-neutral-800 dark:text-white duration-300 transition-all ease-in-out">
    <ul className="menu menu-horizontal bg-neutral-200 card-body text-black rounded-md dark:bg-neutral-800 dark:text-white duration-300 transition-all ease-in-out">
      ...
      <li><a className="active lg:text-sm text-xs" href="/deleteAccount">Delete account</a></li>
    </ul>
    </div>
      ...
      <h3 className="font-bold text-lg">Delete Account</h3>
        <div className="form-control">
          <label className="label">
            <span className=" text-blacklabel-text dark:text-white duration-300 transition-all ease-in-out">Email</span>
          </label>
          <input type="email" defaultValue={currentUser.email} className="input input-bordered bg-neutral-100 border-neutral-300 dark:bg-neutral-500 dark:border-white dark:text-white duration-300 transition-all ease-in-out" required disabled="true"/>
        </div>
        <div className="form-control">
          <label className="label">
            <span className=" text-blacklabel-text dark:text-white duration-300 transition-all ease-in-out">Password</span>
          </label>
        ...
        <div className="form-control mt-6">
          <button className="btn btn-primary bg-red-500 border-red-500 text-white dark:border-red-500 duration-300 transition-all ease-in-out">Delete</button>
        </div>
      </form>
    </div>
  </div>
